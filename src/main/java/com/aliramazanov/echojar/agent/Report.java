...
final class Report {
    ...
    void print(PrintStream out, int override) {
        ...
        List<Finding> echoes = Ledger.findings().stream()
                .filter(finding -> finding.peakPerLease() >= effective)
                .sorted(Comparator.comparingInt(Finding::peakPerLease).reversed())
                .toList();

        if (echoes.isEmpty()) {
            out.printf("%nechojar: no echoes in %d %s%n%n", Ledger.leases(), units);
            spread(out, effective, requests, unit, units);
            caveats(out);
            return;
        }
        ...
        if (sites > 0) {
            out.printf("%nechojar: %s in %s%n%n",
        ...
        for (Finding finding : echoes) {
            ...
            out.printf("    %s in one %s%n", plural(finding.peakPerLease(), "execution", "executions"), unit);
            CallSite site = finding.site();
            out.printf("    %s%n", site == null ? "call site not resolved" : site);
            if (finding.leases() > 1) {
                out.printf("    seen in %d %s, %d executions total%n",
    ...
    private void spread(PrintStream out, int effective, boolean requests, String unit, String units) {
        List<Finding> churn = Ledger.findings().stream()
                .filter(finding -> finding.peakPerLease() < effective)
        ...
        for (Finding finding : churn) {
            out.printf("  %s%n", finding.template().text());
            out.printf("    %s across %d %s%n",
                    plural(finding.totalExecutions(), "execution", "executions"), finding.leases(), units);
            out.printf("    %s%n", finding.site() == null ? "call site not resolved" : finding.site());
            out.println();
        }
        if (requests) {
            out.printf("  these run on nearly every request. that may be correct, or it may be%n");
            out.printf("  a lookup worth caching. a loop would have shown up as an echo above.%n%n");
        } else {
            out.printf("  a loop that reconnects for every query looks like this, and so does a%n");
            out.printf("  query that simply runs once per request. echojar cannot tell them apart%n");
    ...
    private void caveats(PrintStream out) {
        long overflowed = detector.overflowed();
        if (overflowed > 0) {
            out.printf(
                    "  %d statements were not tracked, the template cache is full, raise templates=%n",
                    overflowed);
        }
    ...
    private static Unit unit() {
        ...
        if (dominant(requests, Ledger.leases())) {
    ...
    private void unbounded(PrintStream out) {
        long requests = Diagnostics.snapshot().units();
        long total = Ledger.leases();
        if (requests == 0 || dominant(requests, total)) {
            return;
        }
        out.printf("  only %d of %d units of work were inside a request. the rest ran on%n",
                requests, total);
    ...
    private void health(PrintStream out) {
        Diagnostics.Snapshot health = Diagnostics.snapshot();
        if (health.healthy() && !always) {
            return;
        }
        out.printf("%n  agent health%n");
        out.printf("    %d executions across %d units of work%n", health.executions(), Ledger.leases());
        if (health.units() > 0) {
            out.printf("    %d of those were requests, bounded by a servlet or filter%n", health.units());
        }
        ...
        if (health.units() == 0 && health.leasesClosed() > 20
            ...
            out.printf("    repeated query that borrows a fresh connection would not show as an echo%n");
        }
        out.printf("    %d statements templated, %d types transformed, %d stack walks%n",
                health.statementsTemplated(), health.typesTransformed(), health.stackWalks());
        if (health.typesUnresolvable() > 0) {
            out.printf("    %d types had an incomplete hierarchy, which is normal for optional dependencies%n",
        ...
        health.suppressedBySite().forEach((site, count) ->
                out.printf("      %-10s %6d   first: %s%n", site, count, health.firstFailureBySite().get(site)));
    }
