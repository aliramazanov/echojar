...
final class JdbcInstrumentation {
    ...
    private static final ElementMatcher.Junction<TypeDescription> PREPARED_BY_NAME = named(
            "org.sqlite.jdbc3.JDBC3PreparedStatement");

    private static final ElementMatcher.Junction<TypeDescription> STATEMENT_BY_NAME = named(
        ...
        return agent.type(declares(CONNECTION, config))
                .transform((builder, type, _, module, domain) -> TransformJournal.rewriting(
                        ...
                        .visit(Advice.to(JdbcAdvice.Prepare.class)
                                .on(namedOneOf("prepareStatement", "prepareCall").and(takesArgument(
                                        0,
                                        String.class
                                )).and(isPublic()))));
    }
    ...
    private static @NotNull AgentBuilder preparedFields(@NotNull AgentBuilder agent, EchoConfig config) {
        return agent.type(notYetLoaded(concrete(PREPARED, config)))
                .transform((builder, type, _, module, domain) -> TransformJournal.rewriting(
                        ...
                        .method(named("echojarOwner")).intercept(FieldAccessor.ofField(OWNER_FIELD))
        ...
        return agent.type(declares(PREPARED, config))
                .transform((builder, type, _, module, domain) -> TransformJournal.rewriting(
                        ...
                        .visit(Advice.to(JdbcAdvice.ExecuteBatchPrepared.class)
                                .on(namedOneOf("executeBatch", "executeLargeBatch").and(
                                        takesNoArguments()).and(isPublic())))
    ...
    static ElementMatcher.@NotNull Junction<TypeDescription> globalIgnores(@NotNull EchoConfig config) {
        return nameStartsWith("com.aliramazanov.echojar.shaded.").or(nameStartsWith(
                ...
                .or(nameStartsWith("com.sun.proxy.")).or(nameStartsWith("jdk.proxy"))
                .or(nameContains("$Proxy")).or(nameStartsWith("net.bytebuddy."))
                .or(nameStartsWith("org.mockito.")).or(nameStartsWith("org.easymock."))
                .or(nameContains("$MockitoMock$")).or(nameContains("$$EnhancerBy"))
                .or(nameContains("$$_javassist")).or(nameStartsWith("jdk.internal."))
                .or(nameStartsWith("sun.reflect.")).or(isSynthetic()).or(config.ignoredTypes());
    }
}
